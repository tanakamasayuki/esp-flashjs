...
これらの数値は ESP-IDF が自分の書いたバイト列を読み直して出したものであり、**この fixture について、このプロジェクトが間違えようのない唯一の証言**です。あとから書いたコアダンプのパーサはこれに一致しなければならず、`test/coredump.test.js` がこの5つの値すべてをバイト列に対して検査しています。
