...
| `BAUD` | `115200` | 読み出し速度。**既定を意図的に遅くしてあります**（下記） |
...
**`BAUD` の既定が遅い理由:** 読み出しは、化けた 1 バイトがそのまま fixture としてコミットされてしまう唯一の工程です。ESP32 + CP2102 系のブリッジでは 921600 で失敗することがあるため、既定を 115200 にしています。4MB 全体だと数分かかりますが、バイト列を疑わなくて済むほうが価値があります。急ぐ場合のみ `BAUD=460800` などを指定してください。
