...
| サブタイプ | 形式 | 状況 |
| --- | --- | --- |
| `coredump` (0x03) | ESP-IDF コアダンプ | 未実装。実機 fixture は取得可能になった（`fixture_device` が意図的にクラッシュして書く。[23](#23-未決事項)） |
...
| 5 | **コアダンプの解析** | 未定。ただし従来の障害だった fixture の問題は解消した。`fixture_device` が `coredump` パーティションを持ち、意図的にクラッシュして中身を書くようになったので、他と同じく実機由来の fixture が手に入る。ESP32-S3 での実測: ESP-IDF 自身の `esp_core_dump.h` が記述するとおりの 24 バイトヘッダ、オフセット 24 から `ET_CORE` の ELF、そしてその手前全体に対する CRC-32 が一致する。**自己証明できる**ので、ここで 1.0 に正当に到達しうる数少ない形式である（[9.3](#93-confidence-の基準)）。留意点が2つ。ダンプは**再現しない** — 同一バイナリの2回の実行でどちらも 10884 バイト、うち 205 バイトが相違した。テストは意味とチェックサムを検査してよいがバイト列を期待値にしてはいけない。また、定数ではなく RAM を含む唯一の fixture 領域なので、取得のたびに `check-coredump.mjs` が秘密情報を走査する |
