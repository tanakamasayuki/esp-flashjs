...
- Flash Encryption / Secure Boot の復号・回避。暗号化領域は「暗号化されている」と表示するのみ（[19. 暗号化領域](#20-暗号化領域)）。
- ESP-IDF プロジェクトのビルド。
...
| 8 | **検証手段が用意できるまで Filesystem は読み取り専用** | イメージを書き戻す互換性リスクは読み取りには無い。そして機器を調べられるようにするのは読み取りのほう。ラウンドトリップでは誤魔化せない検査を各形式に用意できた時点で、Phase 4 で再構築を実装した（[12.2](#122-再構築)） |
...
Issue は `{ level: 'error'|'warning', code, params?, partitionIndex? }`。**表示文言ではなく `code` を持つ**（[17.8 i18n](#178-i18n)）。パースは Issue があっても中断せず、可能な限り結果を返す。
...
- 破壊的操作は [18. 安全機構](#18-安全機構) の確認フローを必ず通す。
...
- 既定登録: `partition-table`, `esp-image`, `nvs`, `otadata`, `spiffs`, `littlefs`, `fat`, `coredump`, `raw`。
...
| 値 | 証拠 | 該当箇所 |
| --- | --- | --- |
| 1.0 | magic **と** チェックサム／ハッシュの両方が検証できた | MD5 が一致するパーティションテーブル、checksum が一致するファームウェアイメージ、CRC-32 が一致するコアダンプ |
| 0.95 | その形式に固有の magic 文字列 | LittleFS（superblock が `littlefs` を持つ） |
| 0.8〜0.9 | magic 相当の構造、またはパーティション subtype と、それに矛盾しないレイアウト | FAT（ブートセクタ、0.9）、NVS（subtype、0.9）、チェックサムがこのビルドでは検証できない SHA-256 のコアダンプ（0.9）、SPIFFS（subtype、0.85）、MD5 の無いパーティションテーブル（0.8）、otadata（subtype とサイズ完全一致、0.8） |
| 0.5 | magic は一致するが矛盾がある。あるいは構造は合うが裏付けが無い | MD5 不一致、コアダンプを含む checksum 不一致、テーブルが `spiffs` と呼んでいない領域で見つかった SPIFFS ページ |
...
バッファ自身が語れるのは高エントロピーという事実だけで、それは見かけほど多くを語らない。暗号文と圧縮データはどちらもノイズと区別がつかず（それが両者の目的である）、**バイトカウンタを含むだけの普通のファイル**も同様である。本リポジトリの、暗号化していない ESP32-S3 から取得した LittleFS イメージは、まさにこの理由で 8.0 bits/byte ちょうどを記録する。

したがってエントロピー単独で暗号化を主張してはならない。たいていの場合、より確かな証拠が2つ手元にある。

- **デバイス。** `DeviceInfo.flashEncryptionEnabled` は eFuse 由来である。暗号化が無効だと言っているチップに反論してはならない。そこでの不透明なバイト列は、圧縮済み・ハッシュ済み・元々ランダムのいずれかである
- **パーティションテーブル。** `encrypted` フラグは「フラッシュ暗号化が有効なとき、この領域も暗号化する」という**方針ビット**であって、「このバイト列は暗号文である」という主張ではない。暗号化が無効なチップでは何の意味もないため、それが否定された後にのみ参照する

`classifyEntropy(entropy, ctx)` が両者を統合し、4通りのいずれかを返す。

| 結果 | 条件 | 報告 |
| --- | --- | --- |
| `encrypted` | 高エントロピーで、チップまたはテーブルが暗号化有効と言っている | `type = 'encrypted?'` |
| `possibly-encrypted` | 高エントロピーで、デバイスについて何も分かっていない | `type = 'encrypted?'` |
| `high-entropy` | 高エントロピーだが、チップが暗号化**無効**と言っている | `type = 'raw'`。断定せず事実のみ述べる |
| `unknown` | エントロピーが通常範囲 | `type = 'raw'` |

エントロピーは 16KB 窓ごとの最大値として測る。一様な窓は除外する。最初は数個の窓を標本抽出する実装だったが、**64KB の不透明領域が窓の隙間に落ちて見逃された**。パーティションのどこが書かれているかは偶然なので、全域を走査する。

**「解析できた」ように見せてはならない。**

なお**フラッシュ暗号化は NVS を対象にしない**。チップが「暗号化 有効」と報告することで逆の結論を招きやすいため、暗号化されたデバイスの NVS を見たときは analyzer がその旨を明示する。

### 9.5 復号 — 意図的に実装しない

鍵を受け取ってブラウザ内で復号することは技術的に可能である。WebCrypto に XTS モードは無いが、**AES-CBC を IV=0 で1ブロックだけ暗号化すると、そのブロックについては AES-ECB と等価**であり、これを土台に XTS-AES を構築できる。ランタイム依存は増えない。

実装しない理由は「誰の役に立つか」である。ESP-IDF の既定フローでは鍵はチップ上で生成され、eFuse に焼かれ、read protect される。鍵がデバイスの外に出ることは一度もなく、**本ツールを含め誰もそのフラッシュを復号できない**。復号が意味を持つのは、自分で鍵を生成し鍵ファイルを保管している者だけであり、その者はすでに `espsecure.py` を持っている。

フラッシュ暗号化の有効化はハードウェア上の片道切符でもある。検証用に用意したボードは、以後ほかの用途に使えない。

将来実装する場合に必要なのは、暗号化されたボードではなく**鍵ファイル**である。外部で生成した鍵を焼いた Development モードのデバイスと、焼いたその `.bin` の組み合わせ。この対が無ければ検証のしようがない。
...
3形式とも解析と再構築に対応。同じ型を返すので、UI 側はどの形式を見ているかを知る必要がない。
...
```js
FsStore.from(image)                    // -> FsStore（全ファイルをコピー）
store.write(path, bytes)               // 追加または置換
store.delete(path)                     // ディレクトリは配下ごと
store.rename(from, to)
checkFsStore(store, type)              // -> Issue[]（構築前の事前チェック）
buildFs(store, { size, source })       // -> Uint8Array
```

**Rebuild は「元イメージと同一ジオメトリでの再生成」に限定**し、任意パラメータでの新規生成は行わない。デバイスが想定していないページサイズでフォーマットしたパーティションは、マウントはできてもその後おかしくなる。そしてそれをここから確認する手段が無い。

難しいのはバイト列を書くことではなく、**それが正しいと分かること**である。同じ理解から書かれた builder と parser は、その理解が間違っていても互いに一致する。このプロジェクトはすでにそれで痛い目を見ている（SPIFFS のページフラグを逆に読んでいて、全テストが通っていた）。そこで各形式に、ラウンドトリップ単体では満たせない検査を用意した。

| 形式 | ラウンドトリップでは見えないもの | 代わりに何を検査するか |
| --- | --- | --- |
| SPIFFS | パーサは全ページを走査しオブジェクトインデックスを一切読まない。デバイスはインデックスしか読まない | `readSpiffsViaIndex` がデバイスと同じ経路でデータに到達する。実機キャプチャではパーサと一致するので、生成イメージでも一致すればインデックスは正しい |
| LittleFS | tail チェーンから外れたメタデータペアは問題なく読めてしまい、デバイスは次の書き込みで空き領域として払い出す | `littlefsTraverse` がブロックアロケータと同じ経路を辿り、書いたペアが到達不能なら build を失敗させる |
| FAT | パーティション末尾の wear levelling 状態は、公開された形式のどこにも定義がない | **一切再生成しない。** `buildFat` は元イメージを必須とし、ブートセクタ・予備セクタ・末尾をそのまま引き継ぎ、解析に使ったのと同じマッピングで書き込む |

明示しておくべき帰結が3つある。

- **再構築はコンパクションでもある。** 削除済みページ、上書きされたエントリ、LittleFS ログの履歴はすべて消える。**同じファイルを持つ別のバイト列**になる
- **FAT では空き領域をゼロで埋める。** 実際のファイルシステムは削除時にリンクを外すだけだが、これから公開するかもしれないイメージに削除したファイルの中身を残すべきではない
- **更新日時は保存されない。** SPIFFS は名前の隣に mtime を、ESP-IDF の LittleFS はユーザ属性として持っているが、store がそれを運ばないため、どちらも残らない

**これで確認できないのは「デバイスが実際にマウントするか」である。** それには実機が要る。`tools/hardware-check.mjs --rebuild` がその役目を負う。
...
`coredump` パーティションには、チップが再起動する前に panic ハンドラが書いたものが入っている。ヘッダは ESP-IDF 自身の `esp_core_dump.h` に記述がある。それ以外は3種のチップが書いたダンプで実測し、Espressif の参照デコーダ `esp_coredump` と突き合わせて確認した。

```
 0  4  TOTAL_LEN    フラッシュ上の全長。チェックサムを含む
 4  4  VERSION      (chip id << 16) | (major << 8) | minor
 8  4  TASKS_NUM    ELF 形式では 0 が書かれる
12  4  TCB_SIZE     ELF 形式では 0 が書かれる
16  4  MEM_SEG_NUM  ELF 形式では 0 が書かれる       （v2 / v2.1 のみ）
20  4  CHIP_REV                                     （v2.1 のみ）
 .  .  ET_CORE の ELF イメージ
 .  .  ゼロ埋め
end-4  4  それ以前すべてに対する CRC-32              （または 32 バイトの SHA-256）
```

**ヘッダは 24 バイト固定ではない。** 長さもチェックサム方式もバージョン語から決まり、どちらかを取り違えると ELF の位置がずれてチェックサムも合わなくなる。正常なデバイスが「読めないデータ」に見える、という壊れ方をする。対応表が `COREDUMP_LAYOUTS`:

| バージョン | ヘッダ | チェックサム | 備考 |
| --- | --- | --- | --- |
| 0.1 / 0.2 / 0.3 | 16 / 20 / 24 | CRC-32 | IDF 4.1 より前のバイナリ形式。名前だけ付けて解析はしない |
| 1.0 | 20 | CRC-32 | |
| 1.1 | 20 | SHA-256 | |
| 1.2 | 24 | CRC-32 | 現行の ESP-IDF / Arduino が書く形式 |
| 1.3 | 24 | SHA-256 | |
| 1.4 | 12 | SHA-256 | |

バージョンの上位半分はチップ ID で、ファームウェアイメージのヘッダが持つものと同じである。したがってダンプはそれを書いたシリコンを名乗る。下位半分は `major << 8 | minor` で、major 0 がバイナリ形式、1 が ELF 形式。

ELF の内部では、タスクごとに TCB を持つ `PT_LOAD` と、取得できた範囲のスタックを持つ `PT_LOAD` が並ぶ。`PT_NOTE` セグメントはタスクごとの `NT_PRSTATUS` に加えて、Espressif 独自のノートを3種類運ぶ。

| ノート | 型 | 内容 |
| --- | --- | --- |
| `ESP_CORE_DUMP_INFO` | 8266 | バージョン（再掲）と、アプリ ELF の SHA-256 の先頭部分を NUL 終端文字列で |
| `ESP_EXTRA_INFO` | 677 | クラッシュしたタスクの TCB と、それに続く `(レジスタ ID, 値)` の組 |
| `ESP_PANIC_DETAILS` | 679 | panic メッセージ。例: `abort() was called at PC 0x… on core 1` |

### 13.2 レジスタ

`elf_prstatus` はオフセット 24 の `pr_pid` に TCB アドレスを、オフセット 72 にレジスタセットを置く。そのレジスタセットの並びはアーキテクチャごとに異なり、どちらも自明ではない。

- **Xtensa** は GDB の `xtensa_elf_gregset_t` の順序である。`pc`、`ps`、`lbeg`、`lend`、`lcount`、`sar`、`windowstart`、`windowbase`、予約 56 ワード、そして `ar[0..63]`。ESP-IDF は書き出す前にレジスタウィンドウを正規化するので `windowbase` は 0 であり、`ar[0..15]` がそのタスクの現在の `a0..a15` になる。したがってスタックポインタは `ar[1]` をそのまま読めばよく、ウィンドウの回転を計算する必要はない
- **RISC-V** は素の 32 ワードで、`pc`、`ra`、`sp`、`gp`、`tp` に続いて一時レジスタ・退避レジスタ・引数レジスタが並ぶ

Xtensa については現在のウィンドウの 16 本だけを報告する。残り 48 本は物理レジスタファイルの残りであって、ウィンドウ連鎖を辿らない限り意味を持たない。

`ESP_EXTRA_INFO` は、最初の2組（EXCCAUSE と EXCVADDR）を**位置で**、それ以降をレジスタ ID で読む。参照デコーダがそうしているためである。RISC-V ではこのノートは TCB と終端しか持たないので、例外レジスタは捏造せず「無し」と報告する。

### 13.3 タスク名

FreeRTOS の TCB において `pcTaskName` の位置は固定ではない。その手前に何があるかは `configMAX_TASK_NAME_LEN`、MPU ラッパ、リスト整合性チェックといったビルドオプションで変わる。3つの Arduino コアがたまたま一致している 52 を決め打ちすると、他のビルドではゴミを読み出してそれをタスク名として表示することになる。

代わりに、ダンプ自身に位置を絞り込ませる。1つのダンプに含まれる TCB はすべて同じレイアウトなので、あるオフセットが候補になるのは、取得された**すべての** TCB でそこから NUL 終端の印字可能文字列が始まる場合に限られる。ここで実測した3チップでは、その条件を満たすオフセットがちょうど1つだけ残る。1つも残らなければ `taskNameOffset` は `null` になりタスクは無名になる。それが誠実な結果である。1つ間違った名前は、名前が無いことより悪い。

タスクとスタックの対応付けも、順序ではなく証拠で行う。`pxTopOfStack` は TCB の先頭フィールドで、取得されたスタックの範囲内を指す。したがってスタックセグメントは、その値を含むセグメントである。セグメントの並び順で対応付けると、`CONFIG_ESP_COREDUMP_CAPTURE_DRAM` を有効にしたビルドで間にメモリ領域が挟まった瞬間に破綻する。

### 13.4 意図的にやらないこと

- **バックトレースもシンボル解決もしない。** アドレスを関数名に直すには、クラッシュしたまさにそのビルドの ELF が必要である。ダンプにはそれが入っていない。`esp_coredump` がアプリの ELF を添えて GDB を呼び出すのは、まさにこの理由による。`appElfSha256` ノートが「どのビルドか」を示すので解決は外部で行える。各タスクの PC と SP は、その復元の出発点として報告する
- **SHA-256 のダンプは「未検証」と報告する。** 「有効」とも「壊れている」とも言わない。ここで使える SHA-256 は `crypto.subtle` だけで非同期であり、既定では無効なビルド構成のために parser 全体を非同期化すると呼び出し側すべてに波及する。`checksum.valid` は `null` で、confidence は 0.9 で止まる
- **4.1 以前のバイナリ形式は名前を付けるだけで解析しない。** このプロジェクトが手を出せる範囲でそれを書くものはもう無く、当て推量で本体を解析すれば本物のタスク一覧と見分けがつかなくなる
- **壊れたダンプも表示する。** それを見ている人のデバイスは既に異常を起こしている。チェックサムが1バイトずれたからといってタスク一覧を出さないのは、何をしていたかの唯一の記録を取り上げることになる。confidence を 0.5 に下げ、不一致を報告した上で内容は見せる

### 13.5 テスト

往復検証できるビルダは存在せず、ダンプはクラッシュ時点の RAM そのものである。同一バイナリの2回の実行は、どちらも 10884 バイトで、そのうち 205 バイトが相違した。したがってテストは意味を検査し、バイト列は期待値にしない。

ただしオラクルは通常より良い。クラッシュ後のブートで `fixture_device` は、いま書いたダンプについて ESP-IDF 自身にどう見えるかを尋ねてその答えを出力する。その数値は [tools/fixture-device/README.ja.md](../tools/fixture-device/README.ja.md) に記録してあり、parser はそれに対して検証される。これらのバイト列について、このプロジェクトが間違えようのない唯一の証言である。

fixture が持つ自己整合チェックのうち、2つは名前を挙げておく価値がある。どちらも「それらしく見える間違い」を捕まえるからである。

- 各タスクのスタックポインタは、そのタスク自身の取得済みスタックの内側に入っていなければならない。ポインタはレジスタセットから、範囲はプログラムヘッダから、それぞれ無関係な経路で得られる。gregset のレイアウトか対応付けのどちらかが間違っていれば、両者は一致しない
- すべての `PT_LOAD` はいずれかのタスクに属していなければならない。孤立したセグメントがあれば対応付けが1つ取りこぼしている。それは「ダンプは完全に見えるのに、あるタスクが黙ってスタックを失う」という壊れ方である
...
すべて `code`（`'SYNC_FAILED'` 等の安定文字列）と `details` オブジェクトを持つ。**UI はメッセージ本文ではなく `code` で分岐し、表示文言は `code` から翻訳する**（[17.8](#178-i18n)）。`message` は英語の開発者向け文字列とする。
...
| 名前は分かるが未解析 | subtype が正体を示しているが、パーサが無い | 形式名を含む警告を raw 表示の上に出す。`analyze.notImplemented`、書く予定が無いものは `analyze.neverImplemented` |
...
この表が持つのは予定表ではなく `status` である。以前はロードマップの Phase 番号を持っており、アプリはそれを「Phase 4 で対応予定」と表示していた。Phase 2 / 3 / 4 はいずれも完了済みなので、**どの項目も既に過ぎたリリースを約束していた**ことになる。`nvs_keys` に至っては、決して含まれることのないリリースを約束していた。パーティションを見ている人の前に出すべきものは予定表ではない。「読むつもりがあるかどうか」のほうである。

subtype のヒントは、中身がある領域に対してのみ信用する。空のパーティションは「未対応形式」ではなく「消去済み」として報告する。そうしないと、一度も無線を初期化していないボードの `phy` パーティションが、存在しないデータについての未対応警告を抱えることになる。
...
| `nvs_keys` (0x04) | NVS 暗号化鍵 | **実装しない。** eFuse の鍵が無ければ中身は暗号文であり、原理的に見せられるものが存在しない（[20](#20-暗号化領域)） |
...
- [x] コアダンプの解析（[13](#13-コアダンプ)）— fixture が揃ったので着手。panic 理由、
      クラッシュしたタスク、タスクごとのレジスタとスタックを読み、同じバイト列に
      対する ESP-IDF 自身の申告と突き合わせて検証している
- [ ] ESP8266 サポートの再検討
- [ ] 残る data サブタイプ（`phy`）の解析 — 未着手であり、予定も立てていない。
      それまでの表示は [19](#19-未対応領域の扱い) の通りで、`nvs_keys` は恒久的にここに留まる
...
| 2 | ~~パッケージ分割の是非と時期~~ | **分割しない。** 解析だけの利用者には `esp-flashjs/core` のサブパスで既に小さい方（minified で 120KB に対し 91KB）が届いており、バージョン管理と公開を二重化する理由がない。サブパス export と `sideEffects: false` で得られる以上のものは無く、リリースごとに調整作業が増えるだけ。層を実際に守っているのは `lint:layers` が強制するディレクトリ境界のほう |
...
| 5 | ~~コアダンプの解析~~ | **実装済み**。`src/format/coredump.js`（[13](#13-コアダンプ)）。障害を取り除いたのは fixture である。`fixture_device` が `coredump` パーティションを持ち、意図的にクラッシュして中身を書くので、他の領域と同じく実機由来のダンプが手に入る。**マジックとチェックサムの両方が揃う**ため、1.0 に到達する3つの形式のうちの1つになった（[9.3](#93-confidence-の基準)）。ヘッダの文書からは見えず、作業して初めて分かったことが2つある。ヘッダは 24 バイト固定**ではない**（4種類の長さが流通しており、どれかを教えるのはバージョン語だけである）。そしてタスク名は TCB 内で位置が固定ではないため、決め打ちせず「取得できたすべての TCB が一致するオフセット」として導出している。バックトレースは対象外のままとする。クラッシュしたビルドの ELF が必要で、それはどのダンプにも入っていない |
