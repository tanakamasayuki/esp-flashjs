...
### 3.3 fixture は2種類あり、それぞれ嘘をつく条件が違う

**生成 fixture** は `test/helpers.js` にあります。「何を意図したケースか」が読めることが重要な場合は、バイナリをコミットするより生成するほうが優れています。
...
**実機 fixture** は `test/fixtures/hardware/<チップ>/` にあり、[`tools/fixture-device/`](../tools/fixture-device/README.ja.md) のツールで取得します。

**自分たちが決めた**挙動を固定するには生成 fixture を、**他人が決めた**挙動を固定するには実機 fixture を使ってください。ここで扱うオンフラッシュ形式はすべて ESP-IDF のものであって、このプロジェクトのものではありません。

これは理屈の話ではありません。生成 fixture はパーサが読むのと同じ定数から作られるので、**両方が同時に間違っていても一致します**。このプロジェクトで出荷した／しかけた形式バグは、すべてこの理由でテストを素通りしました。

| バグ | なぜ検出できなかったか |
| --- | --- |
| パーティション magic のバイト順が逆 | parse と build が同じ誤った定数を共有 |
| otadata の CRC が ROM の規約でなく標準 CRC-32 | fixture の CRC も同じ式で生成 |
| SPIFFS の flags を正論理で読んだ | fixture を書く側も同じ |
| SPIFFS の構造体を packed と仮定 | 同上 |
| FAT を wear levelling 抜きで読んだ | 生成イメージにはそのずれが無い |

どれも**まったく妥当に見える出力**を作りました。flags を反転して読んだ SPIFFS は、正しい4つのファイル名と4つの空ファイルを返します。WL を無視した FAT は、ブートセクタを完璧に解析したうえで、FAT テーブルのバイト列を名前にしたファイルを1件返します。どちらも結果を見て「おかしい」とは言えません。

したがって: **このプロジェクトが考案していない形式のパーサには、このプロジェクトが生成していないバイト列に対して走るテストを最低1つ置いてください。** それが不可能なら、生成 fixture で代用してテストスイートを完全に見せるのではなく、不可能である旨をテストに書いてください。

実機 fixture を価値あるものに保つには、2つの習慣が要ります。

- **難所を実際に通っているか確認する。** `/big.bin` は長らく 4096 バイトで、これはちょうど FAT の1クラスタ・LittleFS の1ブロックでした。つまり3つのファイルシステムパーサで最も間違えやすい**連鎖追跡が一度も走っていません**でした。FAT12 の12ビット展開を壊してもユニットテスト1件しか落ちない。20000 バイトにしたら6件落ちるようになりました。
- **わざと壊す。** パーサを書いたら、直したばかりのバグを再導入してスイートが赤くなることを確認してください。今回2回、赤くなりませんでした。どちらも「パーサ自身は依存しないが実機は依存する」不変条件のテストが欠けていました。

実機由来のバイナリを追加する場合は、**MAC アドレス・Wi-Fi 認証情報・証明書・鍵を必ず匿名化**してください。一度コミットすると git 履歴から消すのは面倒です。書き込みスケッチが固定の定数しか書かないのはこのためで、MAC は eFuse にあり取得対象の領域には入りません。

### 3.4 実機に対してライブラリを走らせる

`MockTransport` は実機なしでプロトコルを検証しますが、**それもこのプロジェクトが書いたモック**です。パーサが陥ったのと同じ罠が、1つ下の層にあります。fixture もこれを塞ぎません。fixture は esptool の出力なので、**パーサ**は実バイトで検証されますが、リセットシーケンス・チップ検出・stub ロード・チャンク読み出しは依然としてモック相手にしか動いていませんでした。

```sh
npm run build                       # dist/stub/ に stub が必要
...
Node にシリアル API は無く、このプロジェクトは native 依存を持てないので、`tools/serial-bridge.py` がポートを保持し、Node 側の Transport と stdio で会話します。必要なのは pyserial だけで、しかもこの用途に限られます。

**比較こそが要点です。** esptool とこのライブラリはコードを共有していないので、両者が一致すればそれは自己整合ではなく証拠です。デバイスが書き換えない領域（パーティションテーブル、otadata）は**バイト単位で一致**しなければなりません。NVS とファイルシステムは**内容で**比較します。fixture スケッチが起動のたびに再プロビジョニングするため、ページの seqNo は進み erased エントリは溜まります。ここでバイト一致を要求すると、正常な読み出しを失敗と報告してしまいます。

これでしか見つからなかったものが2つあります。

- **ESP32-P4 はシリコンリビジョンで stub を選ぶ必要がある。** v3.0 未満では RAM の位置が異なり、通常の stub は存在しないアドレスに書かれてチップは一度も挨拶を返しません。**現行の P4 はすべて v3.0 未満**なので、ライブラリはこのファミリで一切動いていませんでした。しかも症状は「原因不明の stub 失敗」です
- **`fetchStub` はブラウザ外では動かない。** モジュール隣の URL を `fetch()` に渡しますが、Node の fetch は `file:` を実装していません。Node から使う場合は `registerStub` が必須で、フラッシュ読み出しはこれに依存します

読み出しが失敗したら、**ライブラリを疑う前に esptool が同じように失敗するか確かめてください。** ここの CH340 基板では、どちらも 320KB を1転送で読めず、チャンクを小さくすればどちらも成功します。これはコードではなくリンクの問題で、議論ではなく確認で決着させるために `--chunk` があります。
